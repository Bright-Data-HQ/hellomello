#!/usr/bin/env python3
"""Analyze Polln SEMrush exports and emit a JSON blob for the deck."""
import csv, json, re, os, collections, urllib.parse, glob

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)                              # competitors/polln
...
POS = _find('polln.com-organic.Positions-*.csv')
PAGES = _find('polln.com-organic.PagesV3-*.csv')
BL = _find('polln.com-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(\bpolln\b|pollin|pollen|polln's)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|herbly|dispensed|easykind|candor|acacia|nuleaf|greencare|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(\bpolln\b|pollin|pollen)", re.I)
...
# subdomain / section split (Polln has www + patient + shop subdomains)
...
for pg in pages:
    ...
    elif host.startswith('shop.'): sub = 'shop'
    elif host.startswith(('www.','polln.com')) or host == 'polln.com': sub = 'www'
...
def bl_type(url):
    u = url.lower()
    if '/library/' in u: return 'Library / Editorial'
    if '/locations/' in u: return 'Locations'
    if '/practitioners/' in u: return 'Practitioners'
    if '/faqs' in u: return 'FAQs'
    if 'patient.polln' in u: return 'Patient portal'
    if 'shop.polln' in u: return 'Shop'
    if u.rstrip('/').endswith('polln.com'): return 'Homepage'
    return 'Other'
...
# ---------- content clusters from page URLs (polln Webflow multi-section + subdomains) ----------
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://(www\.)?polln\.com/?$"),
    ("Patient portal",           r"patient\.polln\.com"),
    ("Shop",                     r"shop\.polln\.com"),
    ("Library (editorial)",      r"polln\.com/library/"),
    ("Locations",                r"polln\.com/locations/"),
    ("Practitioners",            r"polln\.com/practitioners/"),
    ("FAQs",                     r"polln\.com/(faqs|location-faqs|faqs-section)"),
    ("Conditions",               r"polln\.com/condition"),
    ("Pricing / commercial",     r"polln\.com/(pricing|contact|our-doctors|for-clinicians)"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("polln pollin pollen".split())

#!/usr/bin/env python3
"""Analyze Alternaleaf SEMrush exports and emit a JSON blob for the deck."""
import csv, json, re, os, collections, urllib.parse

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)                              # competitors/alternaleaf
DATA = os.path.join(ROOT, 'data')                         # data/data.json, data/notes.json
RAW  = os.path.join(DATA, 'raw')                          # source CSVs live here
PRES = os.path.join(ROOT, 'presentation')                 # presentation html + data.js

POS = os.path.join(RAW, 'alternaleaf.com.au-organic.Positions-au-20260528-2026-05-28T19_52_45Z.csv')
PAGES = os.path.join(RAW, 'alternaleaf.com.au-organic.PagesV3-au-20260528-2026-05-28T19_53_15Z.csv')
BL = os.path.join(RAW, 'alternaleaf.com.au-backlinks_pages.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(alterna ?leaf|alternleaf|alterleaf|alternalead|alternaleafe|altern leaf)"),
    ...
    ("Competitors",      r"(montu|polln|dispensed|herbly|easykind|candor|acacia|nuleaf|greencare|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(alterna ?leaf|alternleaf|alterleaf|alternalead|montu)", re.I)
...
for pg in pages:
    ...
    sub = host.split('.')[0] if host.startswith(('www.','patient.','support.')) else 'www'
    if host.startswith('patient.'): sub = 'patient'
    elif host.startswith('support.'): sub = 'support'
    else: sub = 'www'
...
def bl_type(url):
    u = url.lower()
    if re.search(r'/post/|/blog/', u): return 'Blog / Editorial'
    if '/medical-cannabis' in u or '/medicinal-cannabis' in u: return 'Hub: Medical cannabis'
    if 'patient.alternaleaf' in u: return 'Patient portal'
    if 'support.alternaleaf' in u: return 'Support / help centre'
    if u.rstrip('/').endswith('alternaleaf.com.au'): return 'Homepage'
    return 'Other'
...
# ---------- content clusters from page URLs ----------
# Group all www.alternaleaf.com.au pages by URL pattern.
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://www\.alternaleaf\.com\.au/?$"),
    ("Pricing & promos",         r"alternaleaf\.com\.au/(pricing|promotion-terms|terms/|concession)"),
    ("Reviews",                  r"alternaleaf\.com\.au/reviews"),
    ("Contact / about",          r"alternaleaf\.com\.au/(contact-us|about-us|doctors|process|faq)"),
    ("Wellness hub (/hub/)",     r"alternaleaf\.com\.au/hub/"),
    ("Editorial blog (/post/)",  r"alternaleaf\.com\.au/post/"),
    ("Medical cannabis hub",     r"alternaleaf\.com\.au/(medical-cannabis|medicinal-cannabis)"),
    ("Patient portal",           r"patient\.alternaleaf\.com\.au"),
    ("Support / help centre",    r"support\.alternaleaf\.com\.au"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("alternaleaf alterna leaf altern alternleaf alterleaf alternalead montu".split())

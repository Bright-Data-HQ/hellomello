#!/usr/bin/env python3
"""Analyze Healing Leaves exports and emit a JSON blob for the deck."""
...
POS = _find('healingleaves.com.au-organic.Positions-*.csv')
PAGES = _find('healingleaves.com.au-organic.PagesV3-*.csv')
BL = _find('healingleaves.com.au-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(healing ?leaves|\bhealingleaves\b)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|herbly|polln|dispensed|easykind|candor|acacia|nuleaf|greencare|horizon|econohealth|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(healing ?leaves|\bhealingleaves\b)", re.I)
...
for pg in pages:
    ...
    elif host.startswith('www.') or host == 'healingleaves.com.au': sub = 'www'
...
def bl_type(url):
    u = url.lower()
    if '/regions/' in u or 'telehealth-clinic' in u: return 'Locations'
    if '/compare/' in u: return 'Comparison'
    if '/education' in u: return 'Education'
    if u.rstrip('/').endswith('healingleaves.com.au'): return 'Homepage'
    return 'Other'
...
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://(www\.)?healingleaves\.com\.au/?$"),
    ("Comparison pages",         r"healingleaves\.com\.au/compare/"),
    ("Regions / locations",      r"healingleaves\.com\.au/(regions/|.*telehealth-clinic)"),
    ("Education",                r"healingleaves\.com\.au/education"),
    ("FAQs",                     r"healingleaves\.com\.au/faqs"),
    ("Contact / about",          r"healingleaves\.com\.au/(contact|about)"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("healing leaves healingleaves".split())

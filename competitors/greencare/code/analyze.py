#!/usr/bin/env python3
"""Analyze Greencare exports and emit a JSON blob for the deck."""
...
POS = _find('greencare.com.au-organic.Positions-*.csv')
PAGES = _find('greencare.com.au-organic.PagesV3-*.csv')
BL = _find('greencare.com.au-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(\bgreencare\b|green care|folium)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|herbly|polln|dispensed|easykind|candor|acacia|nuleaf|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(\bgreencare\b|green care)", re.I)
...
for pg in pages:
    ...
    elif host.startswith('www.') or host == 'greencare.com.au': sub = 'www'
...
def bl_type(url):
    u = url.lower()
    if '/pages/folium' in u: return 'Dispensary'
    if '/pages/' in u: return 'Treatments'
    if 'app.candormedical' in u: return 'App / Portal'
    if u.rstrip('/').endswith('greencare.com.au'): return 'Homepage'
    return 'Other'
...
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://(www\.)?greencare\.com\.au/?$"),
    ("Dispensary",               r"greencare\.com\.au/pages/folium"),
    ("Treatments",               r"greencare\.com\.au/pages/"),
    ("Health summary",           r"greencare\.com\.au/healthsummary"),
    ("Wellness blog",            r"greencare\.com\.au/(guide|tips|benefit|health|yoga|meditation|burnout|stretch|hair|ice|sauna|wellbeing|calm|peace)"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("greencare green care folium".split())

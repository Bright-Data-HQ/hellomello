#!/usr/bin/env python3
"""Analyze Acacia Clinic exports and emit a JSON blob for the deck."""
...
POS = _find('acacia-medical.com-organic.Positions-*.csv')
PAGES = _find('acacia-medical.com-organic.PagesV3-*.csv')
BL = _find('acacia-medical.com-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(\bacacia\b|acacia medical|acacia-medical)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|herbly|polln|dispensed|easykind|candor|nuleaf|greencare|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(\bacacia\b)", re.I)
...
for pg in pages:
    ...
    elif host.startswith('www.') or host == 'acacia-medical.com': sub = 'www'
...
def bl_type(url):
    u = url.lower()
    if '/pharmacies' in u: return 'Pharmacies'
    if 'compassionate-access' in u: return 'Compassionate access'
    if u.rstrip('/').endswith('acacia-medical.com'): return 'Homepage'
    return 'Other'
...
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://(www\.)?acacia-medical\.com/?$"),
    ("Pharmacies",               r"acacia-medical\.com/pharmacies"),
    ("Compassionate access",     r"acacia-medical\.com/compassionate-access"),
    ("Pricing / commercial",     r"acacia-medical\.com/(pricing|product)"),
    ("Contact / about",          r"acacia-medical\.com/(contact|about|faq)"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("acacia medical".split())

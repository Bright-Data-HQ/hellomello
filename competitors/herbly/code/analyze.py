#!/usr/bin/env python3
"""Analyze Herbly SEMrush exports and emit a JSON blob for the deck."""
import csv, json, re, os, collections, urllib.parse, glob

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)                              # competitors/herbly
...
POS = _find('herbly.com.au-organic.Positions-*.csv')
PAGES = _find('herbly.com.au-organic.PagesV3-*.csv')
BL = _find('herbly.com.au-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(\bherbly\b|herbley|herbley?s)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|polln|dispensed|easykind|candor|acacia|nuleaf|greencare|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(\bherbly\b|herbley)", re.I)
...
# subdomain / section split (Herbly is single-domain on Shopify)
subdomain_split = collections.defaultdict(lambda: {'traffic':0.0, 'pages':0})
for pg in pages:
    path = urllib.parse.urlparse(pg['url']).path
    if path.startswith('/pages/'): sub = '/pages (city dispensaries)'
    elif path.startswith('/blogs'): sub = '/blogs (editorial)'
    elif path.startswith('/products'): sub = '/products'
    elif path in ('', '/'): sub = 'homepage'
    else: sub = 'other'
...
def bl_type(url):
    u = url.lower()
    if '/pages/' in u and 'dispensary' in u: return 'City dispensary page'
    if '/blogs/' in u or '/blog/' in u: return 'Blog / Editorial'
    if u.rstrip('/').endswith('herbly.com.au'): return 'Homepage'
    return 'Other'
...
# ---------- content clusters from page URLs (Shopify storefront — /pages/ + /blogs/) ----------
CLUSTER_RULES = [
    ("Homepage",                  r"^https?://(www\.)?herbly\.com\.au/?$"),
    ("City dispensary pages",     r"herbly\.com\.au/pages/[a-z\-]+-dispensary"),
    ("About / doctors / process", r"herbly\.com\.au/pages/(about|doctors|plant-medicine-doctors|process|faq|how-it-works|contact)"),
    ("Conditions / education",    r"herbly\.com\.au/pages/(anxiety|sleep|pain|adhd|ptsd|conditions|education|guide)"),
    ("Other landing pages",       r"herbly\.com\.au/pages/"),
    ("Blog / news",               r"herbly\.com\.au/blogs/"),
    ("Products / catalog",        r"herbly\.com\.au/products/"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("herbly herbley herbal".split())

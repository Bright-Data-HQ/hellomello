#!/usr/bin/env python3
"""Analyze Easykind exports and emit a JSON blob for the deck."""
import csv, json, re, os, collections, urllib.parse, glob

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)                              # competitors/easykind
...
POS = _find('easykind.com.au-organic.Positions-*.csv')
PAGES = _find('easykind.com.au-organic.PagesV3-*.csv')
BL = _find('easykind.com.au-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(\beasykind\b|easy kind|easykind)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|herbly|polln|dispensed|candor|acacia|nuleaf|greencare|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(\beasykind\b|easy kind)", re.I)
...
# subdomain / section split (Easykind: www + app + portal subdomains)
...
for pg in pages:
    ...
    elif host.startswith('portal.'): sub = 'portal'
    elif host.startswith('www.') or host == 'easykind.com.au': sub = 'www'
...
def bl_type(url):
    ...
    if 'app.easykind' in u or 'portal.easykind' in u: return 'App / Portal'
    if u.rstrip('/').endswith('easykind.com.au'): return 'Homepage'
    return 'Other'
...
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://(www\.)?easykind\.com\.au/?$"),
    ("App / patient portal",     r"(app|portal)\.easykind\.com\.au"),
    ("Conditions / education",   r"easykind\.com\.au/(chronic-pain|patient-portal-guide|.*-101)"),
    ("Pricing / commercial",     r"easykind\.com\.au/(pricing|careers)"),
    ("Contact / about",          r"easykind\.com\.au/(contact|about|faq)"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("easykind easy kind".split())

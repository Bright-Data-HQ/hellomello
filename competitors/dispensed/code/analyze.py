#!/usr/bin/env python3
"""Analyze Dispensed exports and emit a JSON blob for the deck."""
...
POS = _find('dispensed.com.au-organic.Positions-*.csv')
PAGES = _find('dispensed.com.au-organic.PagesV3-*.csv')
BL = _find('dispensed.com.au-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(\bdispensed\b|dispensed)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|herbly|polln|candor|easykind|acacia|nuleaf|greencare|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(\bdispensed\b)", re.I)
...
# subdomain / section split (Dispensed: www + app + dev/qa subdomains)
...
for pg in pages:
    ...
    elif host.startswith('www.') or host == 'dispensed.com.au': sub = 'www'
...
def bl_type(url):
    u = url.lower()
    if '/location/' in u: return 'Locations'
    if '/blog/' in u: return 'Blog'
    if 'app.dispensed' in u: return 'App / Portal'
    if u.rstrip('/').endswith('dispensed.com.au'): return 'Homepage'
    return 'Other'
...
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://(www\.)?dispensed\.com\.au/?$"),
    ("App / patient portal",     r"app\.dispensed\.com\.au"),
    ("Locations",                r"dispensed\.com\.au/location/"),
    ("Blog / education",         r"dispensed\.com\.au/blog/"),
    ("Pricing / commercial",     r"dispensed\.com\.au/(info/pricing|reviews)"),
    ("Contact / about",          r"dispensed\.com\.au/(info/contact|about|faq)"),
]
...
""".split())
PILLAR_BRAND_TOKENS = set("dispensed astrid".split())

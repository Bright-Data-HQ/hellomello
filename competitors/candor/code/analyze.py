#!/usr/bin/env python3
"""Analyze Candor Medical SEMrush exports and emit a JSON blob for the deck."""
...
POS = _find('candormedical.com-organic.Positions-*.csv')
PAGES = _find('candormedical.com-organic.PagesV3-*.csv')
BL = _find('candormedical.com-backlinks_pages*.csv')
NOTES = os.path.join(DATA, 'notes.json')

# ---------- topic taxonomy ----------
# tuples of (topic, regex). first match wins.
TOPICS = [
    ("Brand",            r"(\bcandor\b|candour|camdor|candor medical|candormedical)"),
    ...
    ("Competitors",      r"(alterna ?leaf|montu|herbly|polln|dispensed|easykind|acacia|nuleaf|greencare|horizon|econohealth|healing leaves|cannatrek|little green pharma|honahlee|releaf|hellomello|mello)"),
]
...
# ---------- brand vs non-brand ----------
brand_re = re.compile(r"(\bcandor\b|candour|camdor|candormedical)", re.I)
...
for pg in pages:
    ...
    elif host.startswith('www.') or host == 'candormedical.com': sub = 'www'
...
def bl_type(url):
    ...
    if u.rstrip('/').endswith('candormedical.com'): return 'Homepage'
    return 'Other'
...
CLUSTER_RULES = [
    ("Homepage",                 r"^https?://(www\.)?candormedical\.com/?$"),
    ("App / patient portal",     r"app\.candormedical\.com"),
    ("Treatments",               r"candormedical\.com/treatments/"),
    ("Education / Blog",         r"candormedical\.com/education/"),
    ("Pricing / commercial",     r"candormedical\.com/(pricing|pricing-guide)"),
...
""".split())
PILLAR_BRAND_TOKENS = set("candor candour candormedical camdor medical".split())
